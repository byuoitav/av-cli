...
func (s *Server) Swab(id *ID, stream AvCli_SwabServer) error {
	...
	case 1:
		...
		for i := range rooms {
			go func() {
				devices, err := db.GetDevicesByRoom(rooms[i].ID)
				if err != nil {
				...
				if len(devices) == 0 {
					err = fmt.Errorf("no devices found in %s", rooms[i].ID)
					c <- SwabResult{
						Id:    rooms[i].ID,
						Error: err.Error(),
					}
				...
				//probably need to add a mutex or something here
				for x := range devices {
					go func() {
						err := swabDevice(context.TODO(), devices[x].Address)
						if err != nil {
							c <- SwabResult{
								Id:    devices[x].ID,
								Error: err.Error(),
	...
	case 2:
		...
		for i := range devices {
			go func() {
				err := swabDevice(context.TODO(), devices[i].Address)
				if err != nil {
					c <- SwabResult{
						Id:    devices[i].ID,
						Error: err.Error(),
...
func (s *Server) Float(id *ID, stream AvCli_FloatServer) error {
	...
	case 1:
		...
		for i := range rooms {
			go func() {
				...
				//maybe need a mutex for the stream
				for x := range devices {
					go func() {
						err := floatShip(devices[x].ID, id.Designation)
						if err != nil {
							c <- FloatResult{
								Id:    devices[x].ID,
								Error: err.Error(),
	...
	case 2:
		...
		for i := range devices {
			go func() {
				err := floatShip(devices[i].ID, id.Designation)
				if err != nil {
					c <- FloatResult{
						Id:    devices[i].ID,
						Error: err.Error(),
